...
export class Api {
  ...
  async getTokenTransfers(accountSpec: AccountSpec, contractAddress: string) {
    ...
    const q = encodeURIComponent(`(from:${address} OR to:${address}) AND (address:${smartContractAddress})`);
    ...
    try {
      ...
      const data = await response.json();
      if (!data.hits) return [];
      console.log(`[track token transfers] Got ${data.hits.length} (of ${data.total}) txs for ${address} coin ${contractAddress}.`);
