<template>
  <div class="detail-balance">
    ...
    <span class="account-balance" :style="{fontSize}">
      <FormattedToken :value="account.data.balance" v-if="account && account.data && !isArg" :decimalsIfAergo="3" />
      <FormattedToken :value="balances.objects[0].token.meta.balance+ ' aer'" v-else-if="balances.objects[0] && isArg" :forcedUnit="'ARG'" :decimalsIfAergo="3"/>
      <span v-else>...</span>
    </span>
