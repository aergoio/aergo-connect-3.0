				...
				<ul class="networks-list">
					...
					>
						<span class="chain-id-icon">
							<Icon
								:name="
									isPublicChainId(network.chainId) ? 'logo' : 'network-other'
								"
								:size="36"
							/>
						</span>
						<div class="network-info">
							...
							<span class="network-actions">
								...
								<span
									class="delete-button"
									@click.prevent.stop="removeNetwork(network.chainId)"
...
export default class AccountsList extends Vue {
	...
	async removeNetwork(chainId: string) {
		if (
			!confirm(`Are you sure you want to remove the custom network ${chainId}?`)
		) {
			return;
		}
