...
export function formatToken(_value: string, unit: string|null = null, prefix = '', decimalsIfAergo = -1, forcedUnit: string|null = null, justDisplay: boolean|false = false) {
    if (justDisplay) {
        // Insert spaces for formatting
        let display = formatNumber(_value, ' ', ' ', unit === 'aergo' ? decimalsIfAergo : -1);
        ...
        const unitHtml = `<span class="unit">${forcedUnit ? forcedUnit : unit}</span>`;
        return `<span class="formatted-value token" title="${_value}">${displayHtml} ${unitHtml}</span>`;
    }
