...
storiesOf("Forms/TextField", module).add("with options", () => ({
  components: { TextField },
  template: `
    <div style="max-width: 400px">
      <TextField :type="type" :variant="variant" :state="state" :placeholder="placeholder" :disabled="disabled" :error="error" v-model="value" />
    </div>`,
  data: () => {
    return {
      value: "",
